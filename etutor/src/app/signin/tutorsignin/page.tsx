...
function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      ...
      setWait("Continue")
     router.push('/etutor') // Redirect to the homepage or another page
    }
  };
  return (
    <div className='relative '>
      ...
      <div className="absolute  hidden custom-2xl:block  z-30 bottom-0 left-0 w-[47rem]">
        ...
        <h1 className="text-3xl 2xl:text-7xl font-extrabold text-darkBlue  lg:text-2xl lg:py-3 mb:text-xl mb:py-2 py-0">
          ...
          <p className="text-lightpurple text-3xl mt-0 custom-lg:mt-3.5">As a eTutor</p>
          <div className="flex items-center justify-center px-6 py-3 sm:py-[17px]   text-2xl gap-3 text-darkBlue cursor-pointer rounded-full bg-transparent border-darkBlue border mt-4 custom-lg:mt-11  mb:text-sm">
          ...
          {error && (
              <p className="text-red-600 mt-2 pl-4">{error}</p>
            )}
          <div></div>
